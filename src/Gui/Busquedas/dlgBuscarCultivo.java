...
public class dlgBuscarCultivo extends javax.swing.JDialog  {
    ...
    private CultivoDTO cultivoSeleccionado;
    ...
    public dlgBuscarCultivo(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        model = (DefaultTableModel) tblCultivo.getModel();
        sorter = new TableRowSorter<>(model);
        tblCultivo.setRowSorter(sorter);
    }
    ...
    private void loadTable(){
        ...
        for (CultivoDTO c : cultivos) {
            Object[] data = {
                ...
                UtilDate.toString(c.getFechaSiembra()),
                UtilDate.toString(c.getFechaEstimCosecha())
            };
            model.addRow(data);
        }
    }  
    ...
    private void btnAceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAceptarActionPerformed
        int row = tblCultivo.getSelectedRow();
        if (row == -1) {
            UtilGui.showErrorMessage(this, "Debe seleccionar un cultivo", "Error");
            return;
        }
        
        int modelRow = tblCultivo.convertRowIndexToModel(row);
        String idCultivo = String.valueOf(model.getValueAt(modelRow, 0));
        
        for (CultivoDTO c : cultivos) {
            if (String.valueOf(c.getId()).equals(idCultivo)) {
